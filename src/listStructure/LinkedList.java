...
public class LinkedList {
	...
	public void insertNode(String data){
		...
		if (head == null) {
			...
			// tempNode.link 는 다음 노드를 참조하고 있으므로,
	...
	public void deleteNode(String data){
		...
		if (data.equals(preNode.getData())) {
			...
			head = preNode.link;
			// preNod의 link는 null을 할당하여 연결을 끊음.
			preNode.link = null;
		} else {
			// tempNode 가 null일 때까지 반복하여 탐색
			while (tempNode != null) {
	...
	public void deleteNode(){
		...
		// head 노드가 link가 null인 겨우
		// 노드가 1개 남앗을 경우
		if (head.link == null){
			// head 에 null을 할당하여 남은 노드와의 연결을 끊음.
			head = null;
		} else {
	...
	public void reverseList(){
		...
		// 1) preNode를 currentNode위치로 이동
		// 2) currentNode는 nextNode 위치로 이동
		// 3) nextNode 는 다음 노드 위치로 이동
		// 4) currentNode의 link는 preNode를 참조하도록 함
		while (nextNode != null){
			preNode = currentNode; // preNode는 currentNode 위치로 이동
			currentNode = nextNode; // currentNode는 nextNode위치로 이동
			nextNode = nextNode.link; // nextNode는 다음 위치로 이동
	...
	public void printList(){
		ListNode tempNode = this.head; // tempNode에 head 가 가리키는 첫번쨰 노드를 할당

		//tempNode 가 null이 아닐 때까지 반복하여 출력
		while (tempNode != null){
